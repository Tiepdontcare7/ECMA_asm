...
const ProductDetail = ({ id }) => {
  axios.get(urlApi)
    .then(({ data }) => {
      ...
      if(item == undefined) router.navigate('/not')

      const cate = data.filter((cate) => {
        return item.category === cate.category && item.id != cate.id
      })
      // console.log(cate);
      ...
      ` 
      document.querySelector('.path-bar').innerHTML = htmlPathbar
              ...
              <div class="border-l-[1px] border-[#ccc] pl-3 pt-5">
                <div class="border- border-b-[1px] pb-3">
                  ...
                  <div class="bg-[#FAFAFA] pl-5 pt-5 pr-7 pb-10 mt-4">
                    <div>
                      <span class="text-3xl text-red-600">${item.list_price} ₫</span>
      ...
      `
      document.querySelector('.detail').innerHTML = htmlDetail;
